...
(() => {
  ...
  function verifyHash(domain, hash, featureSignature) {
    return new Promise((resolve, reject) => {
      try {
        chrome.runtime.sendMessage(
          { action: VERIFY_ENDPOINT_ACTION, domain, hash, featureSignature },
          (response) => {
  ...
  function getVerificationDomain() {
    const hostname = window.location.hostname.trim().toLowerCase();
    if (hostname) return hostname;

    // A file:// page has no hostname. Sending an empty string makes the API
    // reject the payload before it can return a meaningful UNKNOWN result.
    // This sentinel is deliberately *not* an official domain, so a local file
    // can never be marked SAFE simply because it resembles a trusted portal.
  ...
  function buildFeatureSignature(form) {
    const clone = cleanElement(form);
    const tokens = [];
    clone.querySelectorAll("label, input, button, select, textarea, a").forEach((el) => {
      const tag = el.tagName.toLowerCase();
      if (tag === "input" && (el.getAttribute("type") || "text").toLowerCase() === "hidden") return;
      if (tag === "input") {
        tokens.push(`input:${(el.getAttribute("type") || "text").toLowerCase()}:${normalizeFeatureText(el.getAttribute("placeholder"))}`);
      } else {
        tokens.push(`${tag}:${normalizeFeatureText(el.textContent)}`);
      }
    });
    return tokens.join("|");
  }
  ...
  async function scanAndVerify() {
    ...
    for (const form of forms) {
      try {
        ...
        const featureSignature  = buildFeatureSignature(form);
        console.log("[AuthShield] Form fingerprint", { domain, hash, featureSignature });

        const response = await verifyHash(domain, hash, featureSignature);
